...
export default function Birds(){
    ...
    useEffect(() => {
        const fetchSpecies = async () => {
            try {
              const response = await axios.get("http://localhost:5000/birdspecies");
        ...
        const fetchBreeds = async () => {
            try {
              const response = await axios.get("http://localhost:5000/birdbreeds");
    ...
    return(
        <div>
            <Navbar />
            <div>
            <h1 className="text-center text-3xl">Bird Species</h1>
                ...
                <h1 className="text-center text-3xl">Bird Breeds</h1>
                <div className="flex justify-between border border-black mx-10 my-5">
                {breeds.map(b => (
                    <div className="flex flex-col gap-4">
